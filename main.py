...
fastapi_app = FastAPI(title="API Base Ambev", version="5.0.0")
...
# SQL SERVER - GESTOR
# =========================================================
GESTOR_DB_HOST = os.getenv("GESTOR_DB_HOST")
GESTOR_DB_PORT = int(os.getenv("GESTOR_DB_PORT", "1433"))
GESTOR_DB_USER = os.getenv("GESTOR_DB_USER")
GESTOR_DB_PASSWORD = os.getenv("GESTOR_DB_PASSWORD")
GESTOR_DB_NAME = os.getenv("GESTOR_DB_NAME", "master")
...
def get_gestor_conn():
    try:
        return pymssql.connect(
            server=GESTOR_DB_HOST,
            port=GESTOR_DB_PORT,
            user=GESTOR_DB_USER,
            password=GESTOR_DB_PASSWORD,
            database=GESTOR_DB_NAME,
            timeout=5,
            login_timeout=5,
            as_dict=True,
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro conexão SQL Server: {str(e)}"
        )
...
def _query_params_to_where(request: Request):
    ...
    order_dir = (
        params.pop("order_dir", "DESC") or "DESC"
    ).upper()
...
def _select_all(table_name: str, request: Request):
    ...
    try:
        (
            ...
            order_by,
            order_dir
        ) = _query_params_to_where(request)
        ...
        if order_by:
            sql += (
                f" ORDER BY `{order_by}` "
                f"{order_dir if order_dir in ('ASC', 'DESC') else 'DESC'}"
            )
        else:
            sql += " ORDER BY 1 DESC"
...
def _get_produto_packs(produto: str) -> int:
    ...
    try:
        ...
        if not row:
            raise HTTPException(
                status_code=404,
                detail=f"Produto não encontrado: {produto}"
            )
        ...
        if not packs or packs <= 0:
            raise HTTPException(
                status_code=400,
                detail=f"PACKS inválido para o produto {produto}"
            )
...
def _prepare_estoque_payload(
    ...
    if "QUANTIDADE_PLT" in payload:
        payload["QUANTIDADE_PLT"] = _norm_int(
            payload["QUANTIDADE_PLT"],
            0
        )
    ...
    try:
        ...
        if not produto:
            raise HTTPException(
                status_code=400,
                detail="Campo PRODUTO é obrigatório"
            )

        qtd_plt = _norm_int(
            merged.get("QUANTIDADE_PLT"),
            None
        )
        ...
        if qtd_plt is None:
            raise HTTPException(
                status_code=400,
                detail="Campo QUANTIDADE_PLT é obrigatório"
            )

        packs = _get_produto_packs(produto)

        merged["QUANTIDADE_PACK"] = (
            int(qtd_plt) * int(packs)
        )

        if not _norm_text(
            merged.get("CHECKLIST_MASTER")
        ):
...
def root():
    return {
        ...
        "gestor": list(CLIENTES_GESTOR.keys()),
    }
...
# GESTOR - SQL SERVER
# =========================================================
def consultar_estoque_gestor(cdprop: int):

    conn = None
    cursor = None

    try:
        conn = get_gestor_conn()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro consulta gestor: {str(e)}"
        )
...
@fastapi_app.get("/gestor")
def gestor_todos():

    resultado = {}

    for nome, codigo in CLIENTES_GESTOR.items():

        try:
            resultado[nome] = consultar_estoque_gestor(codigo)
...
@fastapi_app.get("/gestor/{cliente_nome}")
def gestor_por_nome(cliente_nome: str):

    cliente_nome = cliente_nome.lower().strip()

    if cliente_nome not in CLIENTES_GESTOR:
    ...
    return consultar_estoque_gestor(
        CLIENTES_GESTOR[cliente_nome]
    )


@fastapi_app.get("/gestor_codigo/{cdprop}")
def gestor_por_codigo(cdprop: int):

    if cdprop not in CLIENTES_GESTOR.values():
    ...
    return consultar_estoque_gestor(cdprop)
...
def atualizar_estoque(item_id: int, data: Dict[str, Any]):
    payload = _prepare_estoque_payload(
        data,
        item_id=item_id
    )
